...
void main() {
  ExecutionRequest request(String provider) => ExecutionRequest(
    missionId: 'M-1', actionId: 'A-1', actionVersion: 3,
    organizationId: 'ORG-1', authorityPassportId: 'P-1', agentId: 'AG-1',
    provider: provider, model: 'model-1', instruction: 'Do bounded work', input: const {},
  );
  ...
  test('provider can change without changing mission authority identifiers', () async {
    final gateway = ExecutionGateway(adapters: {
      'openai': const DemoProviderAdapter('openai', 'gpt-demo'),
      'google': const DemoProviderAdapter('google', 'gemini-demo'),
    });
    ...
    expect(request('openai').authorityPassportId, request('google').authorityPassportId);
  });
  ...
  test('unapproved provider fails closed', () async {
    ...
    expect(() => gateway.execute(request('unknown')), throwsA(isA<ProviderUnavailableException>()));
  });

  test('fallback is provider selection only, never authority expansion', () async {
    final gateway = ExecutionGateway(adapters: {
      'google': const DemoProviderAdapter('google', 'gemini-demo'),
    });
    final result = await gateway.execute(request('openai'));
    expect(result.provider, 'google');
    expect(request('openai').missionId, 'M-1');
    expect(request('openai').authorityPassportId, 'P-1');
  });
}
