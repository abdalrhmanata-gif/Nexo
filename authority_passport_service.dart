...
class AuthorityPassportService {
  ...
  bool verify(AuthorityPassport passport, AuthoritySigner signer, DateTime now) {
    if (passport.status != 'ACTIVE') return false;
    if (now.isBefore(passport.issuedAt) || !now.isBefore(passport.expiresAt)) {
      return false;
    }
    if (passport.keyId != signer.keyId || passport.signatureAlgorithm != signer.algorithm) {
      return false;
    }
  ...
  bool permits({
    ...
    if (passport.status != 'ACTIVE') return false;
    if (passport.organizationId != organizationId || passport.missionId != missionId) {
      return false;
    }
    if (now.isBefore(passport.issuedAt) || !now.isBefore(passport.expiresAt)) return false;
