...
enum ExecutionEnvelopeStatus { prepared, authorized, dispatched, unknown, succeeded, failed, rejected }

enum ExternalOutcomeClassification { notSent, accepted, succeeded, failed, unknown }
...
class ExecutionProtocolValidator {
  ...
  void validate(ExecutionInvocationEnvelope e, {DateTime? now}) {
    ...
    if (e.actionVersion <= 0 || e.actionRevisionBinding != e.bindingKey) {
      throw const ExecutionProtocolViolation('ACTION_REVISION_BINDING_MISMATCH');
    }
    if (e.idempotencyKey.isEmpty || e.inputHash.isEmpty) {
      throw const ExecutionProtocolViolation('MISSING_IDEMPOTENCY_OR_INPUT_HASH');
    }
    if (e.toolId.isEmpty || e.toolVersion.isEmpty || e.agentId.isEmpty || e.provider.isEmpty) {
...
class ExternalOutcomeReceipt {
  ...
  bool get mayRetryAutomatically => classification == ExternalOutcomeClassification.notSent;
  bool get requiresReconciliation => classification == ExternalOutcomeClassification.unknown;
}
