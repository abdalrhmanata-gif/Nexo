...
class MissionKernelSnapshot {
  ...
  MissionKernelSnapshot copyWith({
    ...
    return MissionKernelSnapshot(
      ...
      currentActionRevisionBinding: clearActionRevision
          ? null
          : (currentActionRevisionBinding ?? this.currentActionRevisionBinding),
      executionLeaseId: clearExecutionLease
          ? null
          : (executionLeaseId ?? this.executionLeaseId),
      ...
      verificationResultId: clearVerificationResult
          ? null
          : (verificationResultId ?? this.verificationResultId),
...
class MissionKernel {
  ...
  final Map<String, int> _latestVersions = {};
  final List<MissionKernelEvent> _ledger = [];

  MissionKernel(
      {MissionRuntimeContract runtime = const MissionRuntimeContract()})
      : _runtime = runtime;

  List<MissionKernelEvent> get ledger => List.unmodifiable(_ledger);

  KernelDecision apply(
      MissionKernelSnapshot current, MissionKernelCommand command) {
    if (command.missionId != current.missionId ||
        command.organizationId != current.organizationId) {
      throw const MissionKernelException('TENANT_OR_MISSION_MISMATCH');
    }
    final prior = _processedCommands[command.commandId];
    if (prior != null) return prior;
    final latestVersion = _latestVersions[current.missionId] ?? current.version;
    ...
    if (command.type == MissionKernelCommandType.complete &&
        !command.verificationPassed) {
    ...
    if (command.type == MissionKernelCommandType.recover) {
      ...
      next = current.runtimeState;
    } else if (command.type == MissionKernelCommandType.handoff ||
        command.type == MissionKernelCommandType.selectIntelligence) {
      next = current.runtimeState;
    } else {
    ...
    final updated = current.copyWith(
      ...
      verificationResultId: command.verificationPassed
          ? (command.checkpointId ?? current.verificationResultId)
          : null,
    ...
    _latestVersions[current.missionId] = current.version + 1;
    _ledger.add(event);
    return decision;
  }
  ...
  MissionKernelSnapshot replayEvent(
      MissionKernelSnapshot current, MissionKernelEvent event) {
    if (event.missionId != current.missionId ||
        event.organizationId != current.organizationId) {
      throw const MissionKernelException('TENANT_OR_MISSION_MISMATCH');
    }
    if (event.expectedVersion != current.version ||
        event.resultingVersion != current.version + 1) {
  ...
  static RuntimeEventType _runtimeEvent(MissionKernelCommandType type) {
    switch (type) {
      case MissionKernelCommandType.start:
        return RuntimeEventType.start;
      case MissionKernelCommandType.pause:
        return RuntimeEventType.pause;
      case MissionKernelCommandType.wait:
        return RuntimeEventType.wait;
      case MissionKernelCommandType.requestUser:
        return RuntimeEventType.userRequired;
      case MissionKernelCommandType.resume:
        return RuntimeEventType.resume;
      case MissionKernelCommandType.beginVerification:
        return RuntimeEventType.verify;
      case MissionKernelCommandType.complete:
        return RuntimeEventType.complete;
      case MissionKernelCommandType.fail:
        return RuntimeEventType.fail;
      case MissionKernelCommandType.cancel:
        return RuntimeEventType.cancel;
      ...
      case MissionKernelCommandType.recover:
        throw const MissionKernelException(
            'NO_RUNTIME_EVENT_FOR_CONTROL_COMMAND');
    }
  }

  static void _assertProviderSelectionOnly(MissionKernelCommand command) {
    if (command.actionRevisionBinding != null ||
        command.remainingActions != null ||
        command.remainingBudget != null) {
      throw const MissionKernelException(
          'PROVIDER_SELECTION_CANNOT_MUTATE_AUTHORITY');
    }
  }

  static void _assertNonExpansion(
      MissionKernelSnapshot current, MissionKernelCommand command) {
    if (command.remainingActions != null &&
        command.remainingActions! > current.remainingActions) {
      throw const MissionKernelException('AUTHORITY_EXPANSION_ACTION_BUDGET');
    }
    if (command.remainingBudget != null &&
        command.remainingBudget! > current.remainingBudget) {
      throw const MissionKernelException('AUTHORITY_EXPANSION_SPENDING_BUDGET');
    }
    if (command.actionRevisionBinding != null &&
        current.currentActionRevisionBinding != null &&
        command.actionRevisionBinding != current.currentActionRevisionBinding) {
      throw const MissionKernelException(
          'AUTHORITY_BINDING_CHANGE_REQUIRES_NEW_AUTHORIZATION');
    }
  }

  static void _assertRecoveryBinding(
      MissionKernelSnapshot current, MissionKernelCommand command) {
    if (command.checkpointId == null || command.stateDigest == null) {
      throw const MissionKernelException(
          'RECOVERY_REQUIRES_CHECKPOINT_BINDING');
    }
