...
class LeaseFence {
  ...
  LeaseFenceResult check({
    ...
    if (!lease.isUsableAt(now ?? DateTime.now())) {
      return const LeaseFenceResult(
          allowed: false, reason: 'LEASE_INACTIVE_OR_EXPIRED');
    }
    if (!lease.allowedActions.contains(actionType.toUpperCase())) {
      return const LeaseFenceResult(
          allowed: false, reason: 'ACTION_OUTSIDE_LEASE');
    }
    if (!lease.allowedTools.contains(toolId)) {
      return const LeaseFenceResult(
          allowed: false, reason: 'TOOL_OUTSIDE_LEASE');
    }
    if (lease.actionsRemaining <= 0) {
      return const LeaseFenceResult(
          allowed: false, reason: 'ACTION_BUDGET_EXHAUSTED');
    }
    if (actionRevision <= 0) {
      return const LeaseFenceResult(
          allowed: false, reason: 'INVALID_ACTION_REVISION');
    }
