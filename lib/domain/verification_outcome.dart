...
class VerificationOutcomeProtocol {
  ...
  void validateVerification(VerificationResult result, {DateTime? now}) {
    ...
    if (c.organizationId.isEmpty ||
        c.missionId.isEmpty ||
        c.actionId.isEmpty ||
        c.executionRunId.isEmpty) {
    ...
    if (c.evidenceIds.isEmpty) {
      throw const VerificationOutcomeViolation(
    ...
    if (result.status == VerificationStatus.passed &&
        result.rationale.trim().isEmpty) {
      throw const VerificationOutcomeViolation(
          'PASSED_VERIFICATION_REQUIRES_RATIONALE');
    }
    final t = now ?? DateTime.now().toUtc();
    if (result.resolvedAt.isAfter(t)) {
      throw const VerificationOutcomeViolation(
          'VERIFICATION_TIMESTAMP_IN_FUTURE');
    }
  }

  void validateOutcome(
      OutcomeCommitRequest request, VerificationResult verification) {
    if (request.organizationId != verification.contract.organizationId ||
      ...
      throw const VerificationOutcomeViolation(
          'OUTCOME_VERIFICATION_BINDING_MISMATCH');
    }
    if (!request.verified || request.status != OutcomeStatus.committed) {
      throw const VerificationOutcomeViolation(
          'OUTCOME_NOT_VERIFIED_FOR_COMMIT');
    }
    if (!verification.passed) {
      throw const VerificationOutcomeViolation(
          'OUTCOME_REQUIRES_PASSED_VERIFICATION');
    }
    ...
    if (request.type == OutcomeType.success && request.successScore < 1) {
      throw const VerificationOutcomeViolation(
          'FULL_SUCCESS_REQUIRES_FULL_SCORE');
    }
    if (request.type == OutcomeType.unknown) {
      throw const VerificationOutcomeViolation(
          'UNKNOWN_OUTCOME_CANNOT_BE_COMMITTED_AS_VERIFIED');
    }
  }
