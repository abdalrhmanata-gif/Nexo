...
void main() {
  test('UNKNOWN external outcome forces reconciliation', () {
    final r = const MissionRecoveryPlanner()
        .prepare(seed(), externalOutcome: ExternalOutcomeState.sentUnknown);
  ...
  test('recovery preserves authority and budgets', () {
    final r = const MissionRecoveryPlanner()
        .prepare(seed(), externalOutcome: ExternalOutcomeState.notSent);
  ...
  test('terminal mission does not resume after crash', () {
    ...
    final r = const MissionRecoveryPlanner()
        .prepare(s, externalOutcome: ExternalOutcomeState.confirmedFailure);
  ...
  test('ledger replay rejects version gaps', () {
    ...
    final bad = MissionKernelEvent(
      ...
      commandId: d.event.commandId,
      missionId: d.event.missionId,
      organizationId: d.event.organizationId,
      expectedVersion: 0,
      resultingVersion: 2,
      commandType: d.event.commandType,
      from: d.event.from,
      to: d.event.to,
      actorId: d.event.actorId,
      occurredAt: d.event.occurredAt,
    );
    expect(() => kernel.replayEvent(s, bad),
