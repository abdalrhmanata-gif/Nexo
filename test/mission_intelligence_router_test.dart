...
import 'package:zavqera_followthrough/application/mission_intelligence_router.dart';
import 'package:zavqera_followthrough/domain/mission_intelligence.dart';
...
void main() {
  final profiles = [
    const IntelligenceProfile(
        ...
        estimatedCostPerUnit: 0.8,
        latencyMs: 900,
        riskCeiling: 8),
    const IntelligenceProfile(
        providerId: 'google',
        modelId: 'gemini-demo',
        capabilities: {'reasoning', 'web', 'long-context'},
  ...
  test('routes to the best eligible provider', () {
    final result = MissionIntelligenceRouter(profiles).route(
        const MissionIntelligenceRequest(
            requiredCapabilities: {'reasoning', 'web'},
            maxEstimatedCostPerUnit: 1,
            maxLatencyMs: 1000,
            riskLevel: 6));
  ...
  test('hard constraints reject unsupported capabilities', () {
    expect(
        () => MissionIntelligenceRouter(profiles).route(
            const MissionIntelligenceRequest(
                requiredCapabilities: {'reasoning', 'web', 'computer-use'},
                maxEstimatedCostPerUnit: 2,
                maxLatencyMs: 2000,
                riskLevel: 5)),
        throwsA(isA<IntelligenceRouteDenied>()));
  });
  ...
  test('routing cannot mutate mission authority identifiers', () {
    ...
    MissionIntelligenceRouter(profiles).route(const MissionIntelligenceRequest(
        requiredCapabilities: {'reasoning'},
        maxEstimatedCostPerUnit: 1,
        maxLatencyMs: 1000,
        riskLevel: 3));
