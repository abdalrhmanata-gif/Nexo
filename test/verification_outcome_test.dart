...
OutcomeCommitRequest outcome(
        {OutcomeType type = OutcomeType.success, bool verified = true}) =>
    OutcomeCommitRequest(
      organizationId: 'org1',
      missionId: 'm1',
      goalId: 'g1',
      executionRunId: 'run1',
...
void main() {
  ...
  test('provider success claim is not enough without evidence', () {
    final bad = VerificationResult(
      ...
      status: VerificationStatus.passed,
      confidence: 1,
      rationale: 'Provider said success.',
      resolvedAt: DateTime.utc(2026, 1, 1),
    );
    expect(() => protocol.validateVerification(bad),
  ...
  test('successful outcome requires passed verification', () {
    ...
    final failedVerification = VerificationResult(
      ...
      confidence: 1,
      rationale: 'Evidence contradicts success.',
      resolvedAt: DateTime.utc(2026, 1, 1),
    );
    expect(() => protocol.commit(outcome(), failedVerification),
        throwsA(isA<VerificationOutcomeViolation>()));
  });
  ...
  test('outcome cannot cross mission or execution boundary', () {
    ...
    expect(() => protocol.commit(mismatched, passed()),
        throwsA(isA<VerificationOutcomeViolation>()));
  });
